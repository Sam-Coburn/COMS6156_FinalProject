...
def run!
	...
	$MAX_RESULTS = 10 # intial value
	if (ARGV.length >= 3) # the number of relevant result is given
		$MAX_RESULTS = ARGV[2]
	end
	...
	while count <= results_sorted.size do # while still less than the specified number
		...
		while tpCount < $MAX_RESULTS.to_i do
			x = val[element_count]
			# calculate TP, TF, TN, FN
			simval = x[2].to_f
			if(x[0].split('/')[0] == x[1].split('/')[0]) # same file
	        	tpCount += 1
	        	sum_precision += tpCount.to_f/(element_count+1).to_i
	        	# puts tpCount.to_s + "," + (element_count+1).to_s + "," + sum_precision.to_s
			end
			element_count += 1
		end

		avgPrec = sum_precision.to_f/tpCount.to_i
		# puts "----------"
		# puts avgPrec.to_s
		false_sort_row = []
		...
		false_sort_row[3] = avgPrec.to_s

		sum_r_precision += avgPrec
		# puts "avg-prec," + count.to_s + "," + tpCount.to_s + "," + fpCount.to_s + "," + avgPrec.to_s
...
def check_args
	...
	if ARGV.length == 0
		puts "A Ruby script to calculate Mean Average Precision (MAP) of OCD data set.\r\n"
		puts "==========================================================\r\n"
		puts "Usage: ./map.rb <infile>.csv <outfile> <amount>\r\n"
		...
		puts "<amount> \t\tNumber of relevant documents to calculate MAP\r\n"
		puts "==========================================================\r\n"
		puts "Example: ./map.rb /data/results/diff.csv diff 10\r\n"
		puts "==========================================================\r\n"
		exit
	end
end
